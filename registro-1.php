    ...
    <?php
        ...
        try {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if (empty($user) || empty($pass) || empty($confirm_pass) || empty($email)) {
                    ...
                    if ($count > 0) {
                        ...
                        // Insertamos los datos, incluyendo el correo
                        $stmt = $conn->prepare("INSERT INTO empleados (nombre, password, correo) VALUES (:nombre, :password, :correo)");
                        $stmt->bindParam(':nombre', $user);
                        $stmt->bindParam(':password', $hashed_password);
                        $stmt->bindParam(':correo', $email); // Guardamos el correo
                        $stmt->execute();
